...
const handleSignUpAdmin = async (req, res) => {
  try {
    ...
    if(check) throw new CustomError("Email already exists", 400)
...
const handleSignUpUser = async (req, res) => {
  try {
    ...
    // SEND EMAIL
    const emailRes = await sendMail(user.email, "Emart Registration Complete", `<h1>Hello ${user.name}</h1> <p>Welcome to Emart</p>`)
    if(!emailRes) throw new CustomError("Failed to send email", 500)
...
const handleLogin = async (req, res) => {
  try {
    ...
    if (!await bcrypt.compare(req.body.password, admin.password)) throw new CustomError("Incorrect credentials", 400);
